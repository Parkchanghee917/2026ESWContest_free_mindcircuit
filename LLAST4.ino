...
SoftwareSerial HC06(A1, A2); // A1을 RX, A2를 TX로 설정
    ...
    if (SD.begin(chipSelect)) {
      ...
      if (myFile) {
        ...
        while (myFile.available()) {
          ...
          if (ltr == '1' || ltr == '2') {
            if (lineCount == 1) first = ltr;
            else if (lineCount == 2) second = ltr;
            lineCount++;
          }
        }
        myFile.close();
...
// 노트북 테스트 시 Serial.available(), 블루투스 사용 시 HC06.available()로 변경
  if (HC06.available()) {
    char btSignal = HC06.read();
    if (btSignal == '1') {
      if (first == '1') { led1State = 2; targetLED = 1; }
      else if (second == '1') { led2State = 2; targetLED = 2; }
    } else if (btSignal == '2') {
      if (first == '2') { led1State = 2; targetLED = 1; }
      else if (second == '2') { led2State = 2; targetLED = 2; }
    }
  }
